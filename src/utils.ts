...
export function deleteStorage(...keys: Array<StorageKey>) {
...
export function createUpdater<S>(set: React.Dispatch<React.SetStateAction<S>>) {
...
export function useUpdater<S>(initialState: S | (() => S)): [S, React.Dispatch<Processor<S>>]

export function useUpdater<S>(): [S | undefined, React.Dispatch<Processor<S | undefined>>]

/**
 * 对 React.useState 进行包装，重定义了 setState 函数
 */
export function useUpdater<S>(initialState?: S | (() => S)): [S | undefined, React.Dispatch<Processor<S | undefined>>] {
